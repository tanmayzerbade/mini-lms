...
export default function Bookmarks() {
  ...
  const fetchData = async () => {
    try {
      ...
      const stored = await getStoredIds("bookmarks");
      ...
      const transformed = coursesArray.map((item: any, index: number) => ({
        id: item.id || index,
  ...
  return (
    <>
      <Stack.Screen options={{ title: "Bookmarked Courses" }} />
      <View style={styles.container}>
        {bookmarkedCourses.length === 0 ? (
          <View style={styles.emptyContainer}>
            <Text style={styles.emptyText}>No bookmarked courses yet.</Text>
          </View>
        ) : (
          <FlatList
